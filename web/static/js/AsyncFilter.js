    ...
    try {
      ...
      if(filter_expression && filter_expression!=''){
        try {
           ...
           // get headers listed in the expression
           var tree = compileExpression.parser.parse(filter_expression);
           var js = "";
           function toJs(node) {
           ...
           for(var k in table_dict){
             if(js.indexOf('data["'+k+'"]')>0)
             table4filter[k] = table_dict[k]
           }
        } catch (e) {
          console.error(e)
      ...
      for (var line = 0; line < rows; line++) {
          ...
          var dict = {}
          for(var k in table4filter){
            dict[k] = table4filter[k][line]
          }
          if(filter(dict)){
            isFiltered[line] = 1
            filteredRows = filteredRows + 1
            var f = table_dict.frame[line]
            if(f>frameNumber){
              frameNumber = f
            }
          }
      }
      // var filtered_table_dict = {}
      // for(var k in table_dict){
      //   var arraybuffer = new ArrayBuffer(filtered_indexes.length*4);
      //   filtered_table_dict[k] = new Float32Array(arraybuffer)
      // }
      // for(var k in filtered_table_dict){
      //   for(var i=0; i< filtered_indexes.length; i++){
